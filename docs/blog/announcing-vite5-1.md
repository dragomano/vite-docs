...
head:
  ...
  - - meta
    - property: og:image
      content: https://vite-docs.ru/og-image-announcing-vite5-1.webp
  - - meta
    - property: og:url
      content: https://vite-docs.ru/blog/announcing-vite5-1
  - - meta
...
![Обложка объявления Vite 5.1](/og-image-announcing-vite5-1.webp)
...
Vite продолжает становиться быстрее с каждым новым релизом, и Vite 5.1 содержит множество улучшений производительности. Мы измерили время загрузки 10 тыс. модулей (дерево глубиной 25 уровней) с помощью [vite-dev-server-perf](https://github.com/yyx990803/vite-dev-server-perf) для всех минорных версий, начиная с Vite 4.0. Это хороший бенчмарк для измерения эффекта подхода Vite без сборки бандла. Каждый модуль представляет собой небольшой файл TypeScript со счётчиком и импортами других файлов в дереве, поэтому в основном здесь измеряется время, необходимое для выполнения запросов к отдельным модулям. В Vite 4.0 загрузка 10 тыс. модулей занимала 8 секунд на M1 MAX. В [Vite 4.3, где мы сосредоточились на производительности](./announcing-vite4-3.md), произошёл существенный прорыв, и нам удалось загружать их за 6,35 секунды. В Vite 5.1 мы добились ещё одного значительного скачка производительности. Теперь Vite обслуживает 10 тыс. модулей за 5,35 секунды.

![Прогресс времени загрузки 10K модулей Vite](../images/vite5-1-10K-modules-loading-time.webp)
...
Предварительное объединение во время сборки, экспериментальная функция, добавленная в Vite 3, удалена. С переходом Rollup 4 на встроенный парсер и разработкой Rolldown, как производительность, так и несоответствие между разработкой и сборкой для этой функции больше не актуальны. Мы хотим продолжать улучшать согласованность разработки и сборки и пришли к выводу, что использование Rolldown для «предварительных» и «финальных» сборок является более надёжным решением на будущее. Rolldown также может реализовать кэширование, которое будет гораздо эффективнее во время сборки, чем предварительная упаковка зависимостей. См. ([#15184](https://github.com/vitejs/vite/issues/15184)).
