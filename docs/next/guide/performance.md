...
Использование [`--open` или `server.open`](/config/server-options.html#server-open) также обеспечивает прирост производительности, так как Vite автоматически готовит точку входа вашего приложения или предоставленный URL для открытия.
...
- Используйте CSS вместо Sass/Less/Stylus, когда это возможно (вложенность можно обрабатывать с помощью PostCSS / Lightning CSS).
- Не преобразовывайте SVG в компоненты UI-фреймворков (React, Vue и т. д.). Импортируйте их как строки или URL.
- При использовании `@vitejs/plugin-react` избегайте настройки параметров Babel, чтобы пропустить преобразование во время сборки (будет использоваться только Oxc).

Примеры использования встроенных инструментов:

Хотя ядро Vite основано на нативных инструментах, некоторые функции по умолчанию всё ещё используют ненативные инструменты, чтобы обеспечить лучшую совместимость и более широкий набор возможностей. Но для крупных приложений эти издержки могут быть оправданными.
