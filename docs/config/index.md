...
::: tip СБОРКА КОНФИГУРАЦИИ
По умолчанию Vite использует `esbuild` для сборки конфигурации в временный файл. Это может вызвать проблемы при импорте файлов TypeScript в монорепозитории. Если вы столкнулись с какими-либо проблемами с этим подходом, вы можете указать `--configLoader=runner`, чтобы использовать модульный раннер вместо этого — он не будет создавать временную конфигурацию и будет преобразовывать любые файлы на лету. Обратите внимание, что модульный раннер не поддерживает CJS в конфигурационных файлах, но внешние CJS-пакеты должны работать как обычно.
:::
