...
По умолчанию Vite анализирует все ваши файлы `.html`, чтобы обнаружить зависимости, которые необходимо предварительно собрать (игнорируя `node_modules`, `build.outDir`, `__tests__` и `coverage`). Если указан параметр верхнего уровня [`input`](/config/shared-options#input) или `build.rolldownOptions.input`, Vite вместо этого будет анализировать эти точки входа.
