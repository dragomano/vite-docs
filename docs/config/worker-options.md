...
Функция должна возвращать новые экземпляры плагинов, так как они используются в параллельных сборках воркеров Rolldown. Таким образом, изменения опций `config.worker` в хуке `config` будут проигнорированы.
