...
Если строка начинается с `.`, это позволит использовать это имя хоста без `.`, а также все его поддомены. Например, `.example.com` разрешит доступ к `example.com`, `foo.example.com` и `foo.bar.example.com`.

Если значение установлено в `true`, сервер сможет отвечать на запросы от любых хостов.
Это не рекомендуется, так как это создаст уязвимость для атак с повторным связыванием DNS.
...
Мы рекомендуем устанавливать конкретное значение вместо `true`, чтобы избежать раскрытия исходного кода ненадёжным источникам.
