...
Определяет алиасы, используемые для замены значений в операторах `import` или `require`. Это работает аналогично [`@rollup/plugin-alias`](https://github.com/rollup/plugins/tree/master/packages/alias).
...
Более сложное пользовательское преобразование можно настроить с помощью [плагинов](/guide/api-plugin).

::: warning Использование с SSR
Если вы настроили алиасы для [экстернализованных зависимостей SSR](/guide/ssr.md#ssr-externals), вы можете захотеть создать алиасы для фактических пакетов `node_modules`. Как [Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias), так и [pnpm](https://pnpm.io/aliases/) поддерживают создание алиасов через префикс `npm:`.
:::

### Формат объекта (`Record<string, string>`) {#object-format-record-string-string}

Формат объекта позволяет указывать алиасы как ключ, а соответствующее значение — как фактическое значение импорта. Например:
...
:::warning SSR + ESM
Для сборок SSR дедупликация не работает для выходных данных сборки ESM, настроенных с помощью `build.rolldownOptions.output`. Обходным решением является использование выходных данных сборки CJS, пока ESM не получит лучшую поддержку плагинов для загрузки модулей.
:::
...
Кроме того, условие `style` применяется при разрешении импортов стилей, например `@import 'my-library'`. Для некоторых CSS-препроцессоров также применяются соответствующие им условия, то есть `sass` для Sass и `less` для Less.
...
`paths` применяется только к файлу, который сопоставлен с `tsconfig.json` через его параметры `files` или `include`. Файлы с нестандартными для JS расширениями необходимо явно указывать в них, поскольку обычное значение `include` вроде `"src"` или `"**/*"` соответствует только расширениям TS/JS, что соответствует поведению TypeScript. Например, чтобы использовать алиас `paths` внутри CSS-файла (`@import '@/foo.css'`), укажите эти файлы в `files` или добавьте явное расширение в `include`:
...
::: warning Less не поддерживается
`resolve.tsconfigPaths` не применяется внутри файлов `.less`. Less передаёт Vite только директорию импортирующего файла, а не сам файл, поэтому Vite не может найти соответствующий ему `tsconfig.json`. Используйте относительный путь или [`resolve.alias`](#resolve-alias) для `@import` в Less.
:::
...
::: tip Импорт файлов
Поскольку один и тот же код добавляется в начало файлов в разных директориях, относительные пути не будут корректно разрешаться. Используйте абсолютные пути или [алиасы](#resolve-alias).
:::
...
Включает интеграцию с DevTools для проверки dev-сервера и анализа сборок.

Убедитесь, что `@vitejs/devtools` установлен как зависимость. Установите `@vitejs/devtools-vite` для проверки dev-сервера Vite и `@vitejs/devtools-rolldown` для анализа сборок. По умолчанию DevTools запускается как для `serve`, так и для `build`; используйте `apply`, чтобы ограничить его работу одной из этих команд.

Хуки `config` плагинов не могут изменять параметр `devtools`. Задавайте его непосредственно в пользовательской конфигурации.

После установки `@vitejs/devtools` предоставляет определения типов для этого параметра:
