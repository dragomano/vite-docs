...
Целевой уровень совместимости с браузерами для итоговой сборки. Значение по умолчанию — специальное значение Vite `'baseline-widely-available'`, которое задаёт минимальные версии браузеров, совместимые с [Baseline](https://web-platform-dx.github.io/baseline/) Widely Available на дату, фиксированную для каждого мажорного релиза ([01.01.2026 для этого мажорного релиза](https://web-platform-dx.github.io/supported-browsers/?widelyAvailableOnDate=2026-01-01)). В частности, это `['chrome111', 'edge111', 'firefox114', 'safari16.4', 'ios16.4']`.
...
Если передан колбек, можно вернуть булево значение для выбора включения или исключения. Если ничего не возвращается, применяется логика по умолчанию.
...
Если вы пишете плагин и вам нужно проверять каждый выходной чанк или связанные с ним CSS и статические ресурсы во время сборки, вы также можете использовать API метаданных выходного бандла [`viteMetadata`](/guide/api-plugin#output-bundle-metadata).
...
Установите значение `{}`, чтобы включить наблюдатель Rolldown. Это в основном используется в случаях, связанных с плагинами только для сборки или процессами интеграции.
