...
1. [Предварительное объединение зависимостей](./dep-pre-bundling) для улучшения скорости загрузки страниц и преобразования модулей CommonJS / UMD в ESM. Этап предварительной упаковки выполняется с помощью [esbuild](http://esbuild.github.io/) и делает время холодного старта Vite значительно быстрее, чем у любого сборщика на основе JavaScript.
...
- Для рабочих сборок вы можете выполнить команду `tsc --noEmit` в дополнение к команде сборки Vite.
...
В рабочей сборке файлы `.wasm`, размер которых меньше `assetInlineLimit`, будут вставляться в виде строк base64. В противном случае они будут рассматриваться как [статический ресурс](./assets) и извлекаться по требованию.
...
По умолчанию скрипт воркера будет выдан в виде отдельного блока в рабочей сборке. Если вы хотите вставить воркера в виде строк base64, добавьте запрос `inline`:
