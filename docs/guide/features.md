...
1. [Предварительное объединение зависимостей](./dep-pre-bundling) для улучшения скорости загрузки страниц и преобразования модулей CommonJS / UMD в ESM. Этап предварительного объединения выполняется с помощью [Rolldown](https://rolldown.rs/) и делает время холодного старта Vite значительно быстрее, чем у любого сборщика на основе JavaScript.
...
Vite использует [Oxc Transformer](https://oxc.rs/docs/guide/usage/transformer.html) для транспиляции TypeScript в JavaScript, что быстрее стандартного `tsc`, а HMR-обновления отображаются в браузере менее чем за 50 мс.
...
Vite по умолчанию не транспилирует TypeScript с заданным значением `target`, следуя тому же поведению, что и [esbuild](https://esbuild.github.io/).

Для указания целевой версии (target) в режиме разработки можно использовать опцию [`oxc.target`](/config/shared-options.html#oxc), которая по умолчанию имеет значение `esnext` для обеспечения минимальной транспиляции. При сборке опция [`build.target`](/config/build-options.html#build-target) имеет более высокий приоритет перед `oxc.target` и также может быть настроена при необходимости.
...
Можно указать [`resolve.tsconfigPaths: true`](/config/shared-options.md#resolve-tsconfigpaths), чтобы Vite использовал опцию `paths` из `tsconfig.json` для разрешения импортов.
...
Обратите внимание, что если указаны [`compilerOptions.types`](https://www.typescriptlang.org/tsconfig#types), в глобальную область видимости будут включены только эти пакеты (вместо всех доступных пакетов «@types»). Это рекомендуется начиная с TS 5.9.

::: details Использование директивы с тройной косой чертой
...
Файлы `.jsx` и `.tsx` также поддерживаются из коробки. Транспиляция JSX также осуществляется с помощью [Oxc Transformer](https://oxc.rs/docs/guide/usage/transformer.html).
...
Если вы используете JSX с вашим собственным фреймворком, вы можете настроить пользовательские `jsxFactory` и `jsxFragment`, используя [опцию `oxc`](/config/shared-options.md#oxc). Например, плагин Preact будет использовать:
...
Подробности в [документации Oxc Transformer](https://oxc.rs/docs/guide/usage/transformer/jsx.html).
...
Vite по умолчанию использует [Lightning CSS](https://lightningcss.dev/) для минификации CSS в продакшен-сборках. Однако PostCSS по-прежнему применяется для остальной обработки CSS.

Существует экспериментальная поддержка полного использования Lightning CSS для обработки CSS. Вы можете включить её, добавив [`css.transformer: 'lightningcss'`](../config/shared-options.md#css-transformer).

Для настройки можно передавать опции Lightning CSS через параметр конфигурации [`css.lightningcss`](../config/shared-options.md#css-lightningcss). Для настройки CSS Modules следует использовать [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) вместо [`css.modules`](../config/shared-options.md#css-modules) (который настраивает обработку CSS Modules через PostCSS).
...
- Глобальные шаблоны обрабатываются как спецификаторы импорта: они должны быть либо относительными (начинаться с `./`), либо абсолютными (начинаться с `/`, разрешаемыми относительно корня проекта), либо путями алиасов (см. [опцию `resolve.alias`](/config/shared-options.md#resolve-alias)).
- Глобальное сопоставление выполняется с помощью [`tinyglobby`](https://github.com/SuperchupuDev/tinyglobby) — ознакомьтесь с его документацией для [поддерживаемых глобальных шаблонов](https://superchupu.dev/tinyglobby/comparison).
...
Vite поддерживает импорт предварительно скомпилированных файлов `.wasm` двумя способами: напрямую как [ES-модуль](#esm-integration), если нужны только экспорты модуля, или через [`?init`](#manual-initialization), если требуется явный контроль над созданием экземпляра.

### Интеграция с ES-модулями {#esm-integration}
...
Это соответствует [предложению по интеграции WebAssembly и ES-модулей](https://github.com/WebAssembly/esm-integration). Поскольку экземпляр модуля WebAssembly создаётся асинхронно, напрямую импортированный файл `.wasm` ведёт себя как асинхронный модуль и требует поддержки `await` верхнего уровня.

::: tip Поддержка TypeScript

Поскольку типы файлов `.wasm` неизвестны, TypeScript будет сообщать об ошибках вида `Module '"*.wasm"' has no exported member 'add'`. Чтобы исправить это, включите [`allowArbitraryExtensions`](https://www.typescriptlang.org/tsconfig/#allowArbitraryExtensions) в своём `tsconfig.json` и создайте файл объявления рядом с вашим `.wasm`-файлом. При включённой опции `allowArbitraryExtensions` TypeScript будет искать файл объявления с именем `{имя_файла}.d.wasm.ts` при разрешении импорта `.wasm`. Например, для `add.wasm` создайте файл `add.d.wasm.ts`:
...
Если вам нужен контроль над тем, когда и каким образом создаётся экземпляр модуля, импортируйте его с суффиксом `?init`. Экспорт по умолчанию будет функцией инициализации, возвращающей Promise с объектом [`WebAssembly.Instance`](https://developer.mozilla.org/en-US/docs/WebAssembly/JavaScript_interface/Instance):
...
В продакшен-сборке файлы `.wasm`, размер которых меньше `assetsInlineLimit`, будут вставляться в виде строк base64. В противном случае они будут рассматриваться как [статический ресурс](./assets) и извлекаться по требованию.

::: warning Для сборки под SSR поддерживаются только рантаймы, совместимые с Node.js
...
Обнаружение воркера будет работать только в том случае, если конструктор `new URL()` используется непосредственно внутри объявления `new Worker()`. В противном случае он будет обработан как [URL статического ресурса](./assets#new-url-url-import-meta-url). Кроме того, все параметры опций должны быть статическими значениями (т. е. строковыми литералами).
...
### Оптимизация карты импорта чанков {#chunk-import-map-optimization}
...
Чтобы включить эту функцию, установите [`build.chunkImportMap`](/config/build-options.md#build-chunkimportmap) в `true`.
