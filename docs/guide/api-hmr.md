...
Чтобы модуль мог принимать обновления сам, используйте `import.meta.hot.accept` с колбеком, который получает обновлённый модуль:
...
digraph hmr_boundary {
  ...
  boundary -> edited [label="  импорты" color="${#915930|#f9b44e}" style=bold]
  parent -> boundary [label="  импорты" style=dashed]
  root -> parent [label="  импорты" style=dashed]
}
```
...
if (import.meta.hot) {
  import.meta.hot.accept('./foo.js', (newFoo) => {
    // Колбек получает обновлённый модуль './foo.js'
    newFoo?.foo()
  })
  ...
  import.meta.hot.accept(
    ...
    ([newFooModule, newBarModule]) => {
      // Колбек получает массив, в котором только обновлённый модуль является
...
Зарегистрируйте колбек, который будет вызван, когда модуль больше не импортируется на странице. В отличие от `hot.dispose`, этот колбек можно использовать, если исходный код самостоятельно очищает побочные эффекты при обновлениях, а очистка требуется только при удалении модуля со страницы. В настоящее время Vite использует это для импортов `.css`.
...
Vite создаёт отдельный объект `import.meta.hot.data` для каждого пути модуля. Этот объект сохраняется между последовательными экземплярами одного и того же модуля во время HMR. Изменения, внесённые во время выполнения модуля или через аргумент `data`, переданный в `hot.dispose`, будут доступны следующему экземпляру модуля.

Когда модуль удаляется, колбэки `hot.dispose` и `hot.prune` получают текущий объект данных. После завершения этих колбэков Vite очищает данные. Если позднее модуль импортируется снова, он получает новый пустой объект данных.
...
Удаляет колбек из слушателей событий.
