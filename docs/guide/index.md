...
- Команда сборки, которая объединяет ваш код с [Rolldown](https://rolldown.rs), предварительно настроенным для вывода высоко оптимизированных статических ресурсов для продакшен-версии.
...
<ScrimbaLink href="https://scrimba.com/intro-to-vite-c03p6pbbdq?via=vite" title="Бесплатный курс по Vite на Scrimba">Изучите Vite через интерактивные уроки на Scrimba</ScrimbaLink>

## Поддержка браузеров {#browser-support}

Во время разработки Vite рассчитывает на использование современного браузера, который поддерживает большинство новейших возможностей JavaScript и CSS. Поэтому Vite выбирает [`esnext` в качестве цели преобразования](https://oxc.rs/docs/guide/usage/transformer/lowering.html#target), что позволяет избежать упрощения синтаксиса и обслуживать модули почти в оригинальном виде. Для работы сервера разработки Vite добавляет немного кода времени выполнения, который использует функции из [Baseline](https://web-platform-dx.github.io/baseline/), ставшие доступными к дате последнего крупного обновления (1 января 2026 для этого выпуска).

Для продакшен-сборок Vite по умолчанию ориентируется на версии браузеров, относящиеся к категории [Baseline](https://web-platform-dx.github.io/baseline/) Widely Available на дату, фиксированную для каждого мажорного релиза. Для этого мажорного релиза это соответствует [версиям браузеров, выпущенным примерно в середине 2023 года](https://web-platform-dx.github.io/supported-browsers/?widelyAvailableOnDate=2026-01-01). Целевую версию можно понизить с помощью конфигурации. Кроме того, поддержку устаревших браузеров можно добавить с помощью официального плагина [@vitejs/plugin-legacy](https://github.com/vitejs/vite/tree/main/packages/plugin-legacy). Подробнее см. в разделе [Продакшен-сборка](./build).
...
<ScrimbaLink href="https://scrimba.com/intro-to-vite-c03p6pbbdq/~0yhj?via=vite" title="Создание вашего первого проекта Vite">Посмотреть интерактивный урок на Scrimba</ScrimbaLink>
...
:::: details Создание проекта Vite с помощью командной строки

Можно указать имя проекта и желаемый шаблон с помощью дополнительных параметров командной строки. Например, чтобы создать проект Vite + Vue, выполните команду:
...
Вы также можете использовать такой инструмент, как [tiged](https://github.com/tiged/tiged), чтобы скомпоновать свой проект с одним из шаблонов. Предполагая, что проект находится на GitHub и использует `main` в качестве ветки по умолчанию, вы можете создать локальную копию, используя следующие команды:
...
cd packages/vite
pnpm run build
pnpm link # используйте предпочитаемый менеджер пакетов для этого шага
```

Затем перейдите к проекту, основанному на Vite, и запустите `pnpm link vite` (или менеджер пакетов, который вы использовали для глобального объединения `vite`). Теперь перезапустите dev-сервер, чтобы оказаться на острие атаки!
