...
3. Для продакшен-сборки, после выполнения команды `vite build`, будет сгенерирован файл `.vite/manifest.json` вместе с другими файлами ресурсов. Пример файла манифеста выглядит следующим образом:
   ...
   Манифест сопоставляет исходные файлы с их выходными артефактами сборки и зависимостями:
   ...
   digraph manifest {
     ...
     foo [label="views/foo.js\n(точка входа)" fillcolor="${#e9eaff|#222541}"]
     bar [label="views/bar.js\n(точка входа)" fillcolor="${#e9eaff|#222541}"]
     shared [label="_shared-B7PI925R.js\n(общий чанк)" fillcolor="${#f2ecfc|#2c273e}"]
     baz [label="baz.js\n(динамический импорт)" fillcolor="${#fcf4dc|#38301a}"]
     ...
     logo [label="logo.svg\n(ресурс)" shape=ellipse fillcolor="${#def5ed|#15312d}"]
   ...
   - **Чанки точек входа**: Генерируются из файлов, указанных в [`build.rolldownOptions.input`](https://rollupjs.org/configuration-options/#input). Эти чанки имеют `isEntry: true`, а их ключ — это относительный путь src от корня проекта.
