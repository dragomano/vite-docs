...
3. Для продакшен-сборки, после выполнения команды `vite build`, будет сгенерирован файл `.vite/manifest.json` вместе с другими файлами ресурсов. Пример файла манифеста выглядит следующим образом:
   ...
   - **Чанки точек входа**: Генерируются из файлов, указанных в [`build.rolldownOptions.input`](https://rolldown.rs/configuration-options/#input). Эти чанки имеют `isEntry: true`, а их ключ — это относительный путь src от корня проекта.
