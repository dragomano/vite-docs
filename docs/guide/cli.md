...
| `--configLoader <loader>` | Используйте `bundle` для сборки конфигурации с помощью esbuild, или `runner` (экспериментально) для обработки её на лету, или `native` (экспериментально) для загрузки с помощью встроенной среды выполнения (по умолчанию: `bundle`) |
