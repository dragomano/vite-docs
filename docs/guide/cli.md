...
Эта команда запускает сервер в директории сборки (по умолчанию `dist`). Запустите `vite build` заранее, чтобы убедиться, что директория сборки актуальна. В зависимости от настроенного в проекте [`appType`](/config/shared-options.html#apptype), она использует определённые мидлвары.
