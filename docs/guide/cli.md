...
| `--experimentalBundle`    | Использовать экспериментальный режим полной сборки (это крайне экспериментальная функция) (`boolean`) |
...
| `--configLoader <loader>` | Используйте `bundle` для сборки конфигурации с помощью Rolldown, или `runner` (экспериментально) для обработки её на лету, или `native` (экспериментально) для загрузки с помощью встроенной среды выполнения (по умолчанию: `bundle`) |
| `--profile [name]`        | Запустить встроенный инспектор Node.js и записать профиль в `<name>.cpuprofile` (см. [Узкие места производительности](/guide/troubleshooting#performance-bottlenecks)) (`boolean \| string`) |
...
| Параметры                      |                                                                                                                                                   |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--target <target>`            | Цель транспиляции (по умолчанию: `"baseline-widely-available"`) (`string`)                                                                        |
...
| `--minify [minifier]`          | Включить/выключить минификацию или указать используемый минификатор (по умолчанию: `"oxc"`) (`boolean \| "oxc" \| "terser" \| "esbuild"`)         |
...
| `--configLoader <loader>`      | Используйте `bundle` для объединения конфигурации с помощью Rolldown, или `runner` (экспериментально) для обработки на лету, или `native` (экспериментально) для загрузки с использованием нативной среды выполнения (по умолчанию: `bundle`) |
| `--profile [name]`             | Запустить встроенный инспектор Node.js и записать профиль в `<name>.cpuprofile` (см. [Узкие места производительности](/guide/troubleshooting#performance-bottlenecks)) (`boolean \| string`)            |
| `-d, --debug [feat]`           | Показать журналы отладки (`string \| boolean`)                                                                                                    |
| `-f, --filter <filter>`        | Отфильтровать журналы отладки (`string`)                                                                                                          |
| `-m, --mode <mode>`            | Установить режим окружения (`string`)                                                                                                             |
...
| `-l, --logLevel <level>`  | Уровень предупреждений: info \| warn \| error \| silent (`string`)                                                                                |
| `--clearScreen`           | Разрешить/отключить очистку экрана при ведении журнала (`boolean`)                                                                                |
| `--configLoader <loader>` | Используйте `bundle` для объединения конфигурации с помощью Rolldown, или `runner` (экспериментально) для обработки на лету, или `native` (экспериментально) для загрузки с использованием нативной среды выполнения |
| `-d, --debug [feat]`      | Показать журналы отладки (`string \| boolean`)                                                                                                    |
| `-f, --filter <filter>`   | Отфильтровать журналы отладки (`string`)                                                                                                          |
| `-m, --mode <mode>`       | Установить режим окружения (`string`)                                                                                                             |
| `-h, --help`              | Отобразить доступные параметры                                                                                                                    |
...
Эта команда запускает сервер в директории сборки (по умолчанию `dist`). Запустите `vite build` заранее, чтобы убедиться, что директория сборки актуальна. В зависимости от настроенного в проекте [`appType`](/config/shared-options#apptype), она использует определённые мидлвары.
