...
**Рекомендуется сначала ознакомиться с [документацией по плагинам Rolldown](https://rolldown.rs/apis/plugin-api), прежде чем читать разделы ниже.**

## Создание плагина {#authoring-a-plugin}

Vite стремится предлагать устоявшиеся паттерны из коробки, поэтому перед созданием нового плагина убедитесь, что вы вы ознакомились с разделом [Возможности](/guide/features), чтобы понять, покрывают ли они ваши потребности. Также ознакомьтесь с доступными плагинами сообщества, как в виде [совместимых плагинов Rollup](https://github.com/rollup/awesome), так и [специфичных для Vite плагинов](https://github.com/vitejs/awesome-vite#plugins).
...
Если плагин не использует специфичные для Vite хуки и может быть реализован как [совместимый плагин Rolldown](#rolldown-plugin-compatibility), то рекомендуется использовать [соглашения об именовании плагинов Rolldown](https://rolldown.rs/apis/plugin-api#conventions).
...
Смотрите также [Конвенция виртуальных модулей](https://rolldown.rs/apis/plugin-api#virtual-modules).
...
Виртуальные модули позволяют передавать информацию, полученную во время сборки, в исходные файлы с помощью обычного синтаксиса импорта ESM. Полное описание соглашения об именовании см. в разделе [Соглашение для виртуальных модулей](https://rolldown.rs/apis/plugin-api#virtual-modules).
...
Обратите внимание, что хук [`moduleParsed`](https://rolldown.rs/reference/Interface.Plugin#moduleparsed) **не** вызывается во время разработки, поскольку Vite избегает полного разбора AST для повышения производительности.

[Хуки генерации вывода](https://rolldown.rs/apis/plugin-api#output-generation-hooks) (за исключением `closeBundle`) **не** вызываются во время разработки.
...
Обратите внимание, что это работает отдельно от порядка хуков — они по-прежнему подчиняются своему атрибуту [`order`](https://rolldown.rs/reference/TypeAlias.ObjectHook#order), как обычно для хуков Rolldown.
...
## Совместимость плагинов Rolldown {#rolldown-plugin-compatibility}
...
- Не использует хук [`moduleParsed`](https://rolldown.rs/reference/Interface.Plugin#moduleparsed).
- Не зависит от специфичных для Rolldown опций, таких как [`transform.inject`](https://rolldown.rs/reference/InputOptions.transform#inject).
...
Чтобы отправить события от клиента к серверу, мы можем использовать [`hot.send`](/guide/api-hmr.html#hot-send-event-data):
