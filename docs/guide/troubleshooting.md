...
Можно попробовать обойти проблему, выбрав другое имя для чанка с помощью [`build.rollupOptions.output.chunkFileNames`](../config/build-options.md#build-rollupoptions), так как эти расширения часто блокируют запросы на основе имён файлов (например, имена, содержащие `ad`, `track`).
