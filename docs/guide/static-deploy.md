<!--
  ПРОЧИТАЙТЕ ЭТО, ЕСЛИ ХОТИТЕ ДОБАВИТЬ НОВУЮ ПЛАТФОРМУ ДЛЯ РАЗВЁРТЫВАНИЯ.

  Не стесняйтесь отправлять пулреквест, добавляющий новый раздел со ссылкой на руководство
  по развёртыванию вашей платформы, при условии, что оно соответствует следующим критериям:

  1. Пользователи должны иметь возможность развернуть свой сайт бесплатно.
  2. Бесплатный тариф должен позволять хостить сайт неограниченное время и не иметь временных ограничений.
    Допускается предоставление ограниченного количества вычислительных ресурсов или количества сайтов в обмен на бесплатное размещение.
  3. Ссылки в руководствах не должны содержать вредоносного контента.
  4. Платформа должна функционировать не менее 1 года. Пожалуйста, приведите в описании вашего PR подтверждение этого.

  Новые разделы следует добавлять в конец файла.
  Пожалуйста, ориентируйтесь на существующие разделы в нижней части этого файла — они служат примерами форматирования нового раздела.

  Команда Vite может время от времени изменять эти критерии и проверять текущий список.
  Если какой-либо раздел будет удалён, мы заранее уведомим авторов соответствующего пулреквеста.
-->
...
Вы также можете добавлять пользовательские домены и настраивать параметры сборки на Pages. Узнайте больше об [интеграции Git с Cloudflare Pages](https://developers.cloudflare.com/pages/configuration/git-integration/).
...
По умолчанию любое новое изменение, запушенное в указанную ветку, автоматически запустит новое развёртывание. [Автоматическое развёртывание](https://render.com/docs/deploys#configuring-auto-deploys) можно настроить в настройках проекта.
