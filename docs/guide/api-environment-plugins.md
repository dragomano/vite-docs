...
## Хуки уровня окружения и глобальные хуки {#per-environment-hooks-and-global-hooks}

Плагины выполняются в рамках общего конвейера, однако их хуки делятся на две категории в зависимости от того, вызываются ли они один раз для всего сервера или отдельно для каждого окружения.

Глобальные хуки вызываются только один раз, независимо от настроенных окружений. Они отвечают за задачи, относящиеся ко всему приложению, такие как разрешение конфигурации или настройка серверов разработки и предпросмотра, поэтому `this.environment` для них не имеет смысла. К глобальным относятся хуки, связанные с обработкой конфигурации и работой сервера.

Хуки уровня окружения вызываются отдельно для каждого окружения и предоставляют доступ к текущему окружению через `this.environment` в своём контексте. Все [универсальные хуки](/guide/api-plugin#universal-hooks) являются хуками уровня окружения, как и другие специфичные для Vite хуки, работающие с модулями. Однако обратите внимание, что `buildStart` и `buildEnd` вызываются только для клиентского окружения, если не установлен [флаг `perEnvironmentStartEndDuringDev: true`](#per-environment-state-in-plugins).
...
Пустой объект достаточен для регистрации окружения, при этом используются значения по умолчанию из конфигурации окружения корневого уровня.

## Настройка окружения с помощью хука `configEnvironment` {#configuring-environment-using-the-configenvironment-hook}

- **Тип:** `(name: string, config: EnvironmentOptions, env: { mode: string, command: 'build' | 'serve', isSsrBuild?: boolean, isPreview?: boolean, isSsrTargetWebworker?: boolean }) => EnvironmentOptions | null | void`
...
## Плагины уровня окружения с использованием хука `applyToEnvironment` {#per-environment-plugins-using-the-applytoenvironment-hook}

- **Тип:** `(environment: PartialEnvironment) => boolean | PluginOption | Promise<boolean>`
- **Режим работы:** `async`, `sequential`
- **Область применения:** [Уровень окружения](#per-environment-hooks-and-global-hooks)
...
- **Во время разработки и сборки:** плагины общие, с [фильтрацией по окружению](#per-environment-plugins-using-the-applytoenvironment-hook)
