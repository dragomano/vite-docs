...
Хуки уровня окружения вызываются отдельно для каждого окружения и предоставляют доступ к текущему окружению через `this.environment` в своём контексте. Все [хуки Rolldown](/guide/api-plugin#rolldown-hooks) являются хуками уровня окружения, как и другие специфичные для Vite хуки, работающие с модулями. Однако обратите внимание, что `buildStart` и `buildEnd` вызываются только для клиентского окружения, если не установлен [флаг `perEnvironmentStartEndDuringDev: true`](#per-environment-state-in-plugins).
