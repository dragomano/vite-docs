...
Когда `builder` не равен `undefined` (или при вызове `vite build --app`), `vite build` будет выбирать сборку всего приложения вместо этого. Это станет стандартным поведением в будущем мажорном релизе. Будет создан экземпляр `ViteBuilder` (эквивалент `ViteDevServer` во время сборки) для сборки всех настроенных окружений для рабочего режима. По умолчанию сборка окружений выполняется последовательно, соблюдая порядок записи `environments`. Фреймворк или пользователь могут дополнительно настроить, как окружения будут собираться, используя:
