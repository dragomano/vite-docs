...
Чтобы предотвратить случайную утечку переменных окружения в клиентский код, только переменные, начинающиеся с префикса `VITE_`, доступны в вашем коде, обработанном Vite. Например, для следующих переменных окружения:
...
Только `VITE_SOME_KEY` будет доступна как `import.meta.env.VITE_SOME_KEY` в вашем клиентском исходном коде, но `DB_PASSWORD` не будет доступна.
...
Как показано выше, `VITE_SOME_KEY` является числом, но возвращает строку при парсинге. То же самое произойдёт и с булевыми переменными окружения. Убедитесь, что вы преобразуете их в нужный тип при использовании в вашем коде.
:::
