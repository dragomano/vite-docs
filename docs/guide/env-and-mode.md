...
- **`import.meta.env.PROD`**: {boolean} указывает, работает ли приложение в рабочем режиме (работает ли dev-сервер с `NODE_ENV='production'` или приложение, собранное с `NODE_ENV='production'`).
