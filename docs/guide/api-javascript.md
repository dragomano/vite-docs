...
interface PreviewServer {
  ...
  /**
   * Экземпляр приложения connect.
   * - Может использоваться для подключения пользовательских мидлваров к серверу предпросмотра.
   * - Также может использоваться в качестве функции обработчика для пользовательского http-сервера
   *   или в качестве мидлвара в любых фреймворках Node.js в стиле connect.
   *
   * https://github.com/senchalabs/connect#use-middleware
...
Обратите внимание, что значения `null` и `undefined` в `overrides` пропускаются и не объединяются. Если вам нужно явно очистить значение из `defaults`, измените непосредственно результат `mergeConfig`.

::: tip ПРИМЕЧАНИЕ
`mergeConfig` принимает конфигурацию только в форме объекта. Если у вас есть конфигурация в форме колбека, вы должны вызвать её перед передачей в `mergeConfig`.

Вы можете использовать вспомогательную функцию `defineConfig`, чтобы объединить конфигурацию в форме колбека с другой конфигурацией:
...
Версия Rolldown, используемая в Vite, в виде строки (например, `"1.0.0"`). Это реэкспорт константы [`VERSION`](https://rolldown.rs/reference/Variable.VERSION) из пакета `rolldown`.

## `esbuildVersion`

**Тип:** `string`

Сохранено исключительно для обратной совместимости.
