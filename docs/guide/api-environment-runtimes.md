...
Эта страница предназначена для провайдеров рантаймов — авторов, которые интегрируют JavaScript-рантайм с Vite. Под рантаймом здесь понимается JavaScript-движок, в котором выполняется трансформированный код: Node.js, браузер, Cloudflare workerd или Worker thread. Провайдер рантайма упаковывает интеграцию для одного из этих рантаймов, чтобы авторам фреймворков и конечным пользователям (разработчикам приложений) не приходилось настраивать всё самостоятельно.

## Фабрики окружений {#environment-factories}

Фабрики окружений предназначены для реализации провайдерами рантаймов, а не конечными пользователями. Фабрики окружений возвращают `EnvironmentOptions` для наиболее распространённого случая — использования целевого рантайма как для dev-, так и для build-окружений. Также можно задать параметры окружения по умолчанию, чтобы пользователю не приходилось делать это самостоятельно.
...
По умолчанию к транспортам `HotChannel` применяются ограничения `server.fs`, что означает, что могут быть выданы только файлы из разрешённых директорий. Если ваш транспорт не открыт для сети (например, он взаимодействует через рабочие потоки или внутрипроцессные вызовы), вы можете установить `skipFsCheck: true` в `HotChannel`, чтобы обойти эти ограничения.
...
function createWorkerEnvironment(name, config, context) {
  ...
  const workerHotChannel = {
    // Сообщения рабочих потоков не доступны по сети, проверки server.fs пропускаются
    skipFsCheck: true,
...
Для SSR-окружения, которое выполняется в том же процессе Node.js, что и сервер Vite, Vite экспортирует готовый `HotChannel` — функцию `createServerHotChannel`:
